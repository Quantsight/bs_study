...
class TSCV:
    ...
    def __init__(self, ts, tr_days, ts_days):
        ...
        :param ts: pandas index
        :param tr_days: number of days in training windows
        :param ts_days: number of days in testing windows
        """
        ts = ts.order()
        n = len(ts)
        ...
        if tr_beg >= ts[0]:  
            ...
            # Iteratively move training window back from last possible to
            # first possible (and still start after first encountered date)
            while tr_beg >= ts[0]:
                # move beginning of training window back <ts_days>
                tr_beg = tr_beg + pd.tseries.offsets.relativedelta(days=-ts_days)

                # set beginning of testing window to be <tr_days> after <tr_beg>
                ts_beg = tr_beg + pd.tseries.offsets.relativedelta(days= tr_days)

                # set end of testing window to be <ts_day> after <ts_beg>
    ...
    def __call__(self):
        """
        generate in-order train/tests subsets
        """
        for tr_beg, ts_beg, ts_end in self.endpoints:
            tr_candidates = self.index[(self.index >= tr_beg) & (self.index < ts_beg)]
            ts_candidates = self.index[(self.index >= ts_beg) & (self.index < ts_end)]
            yield tr_candidates, ts_candidates

    def next_random(self, N):
        """
        generate cross-validation random subsets (without replacement; unique
        train & test row indices)
        """
        p = (N - 1.0)/N if N > 1 else 1.0
        for tr_beg, ts_beg, ts_end in self.endpoints:
            for i in range(N):
                # dates that are in selected training range:
                tr_candidates = self.index[(self.index >= tr_beg) & (self.index < ts_beg)]

                # ... and a random subset of their corresponding rows:
                tr_rows = list(set(random.sample(tr_candidates, int(p*len(tr_candidates)))))

                ts_candidates = self.index[(self.index >= ts_beg) & (self.index < ts_end)]
                ts_rows = list(set(random.sample(ts_candidates, int(p*len(ts_candidates)))))
...
if __name__ == '__main__':
    ...
    parser.add_argument('tr_days', type=int)
    parser.add_argument('ts_days', type=int)
    ...
    cv = TSCV(X.index, tr_days=args.tr_days, ts_days=args.ts_days)

    repeats = 5
    for tr_rows, ts_rows, tr_beg, ts_beg in cv.next_random(repeats):
        print tr_beg, ts_beg

    print('_'*80)
    cv = TSCV(X.index, tr_days=args.tr_days, ts_days=args.ts_days)
    for tr_dates, ts_dates in cv():
        print tr_dates.values, ts_dates.values
